...
if __name__ == "__main__":
    ...
    sc.setCheckpointDir('/home/tmp/check2')
    ssc = StreamingContext(sc, 5)

    print("3. kafka stream")

    # ds1 = KafkaUtils.createStream(ssc, "192.168.16.250:22181", "test-consumer-group", {"syslog.cef.original": 5})
    ds1 = KafkaUtils.createStream(ssc, "192.168.2.12:2181", "test-consumer-group11", {"suricata12": 1})
    ...
    dns_cnt = dns.filter(lambda x: x['event_type'] == 'dns')
    dns_type = dns_cnt.filter(lambda x: x['dns']['type'] == 'query')
    # dns_tot = dns_type.updateStateByKey(updateTotalCnt)
    dns_type.foreachRDD(hello)
